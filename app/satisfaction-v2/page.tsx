...
export default function SatisfactionV2Dashboard() {
  ...
  return <main className="satv2-page"><div className="satv2-shell">
    <div className="satv2-note"><strong>V2 TEST MODE</strong><span>ทุกฟิลด์ถูกจำแนกและประมวลผลตามชนิดข้อมูลอัตโนมัติ · ระบบเดิม `/satisfaction` ไม่ถูกแก้ไข</span><Link href="/admin/satisfaction-v2"><Settings2 size={16}/> ตั้งค่า V2</Link></div>
    ...
    <section className="satv2-toolbar"><div><CalendarDays size={16}/><select value={selectedDate} onChange={(e) => setSelectedDate(e.target.value)}><option value="all">ทุกวันที่</option>{dates.map((d) => <option key={d} value={d}>{thaiDateLabel(d)}</option>)}</select></div><div><Building2 size={16}/><select value={selectedOrg} onChange={(e) => setSelectedOrg(e.target.value)}><option value="all">ทุกหน่วยงาน</option>{organizations.map((org) => <option key={org} value={org}>{org}</option>)}</select></div><button onClick={() => refresh()} disabled={loading}><RefreshCw className={loading ? 'spin' : ''} size={16}/> อัปเดต</button><ExportMenu source={source} headers={sheetData.headers} rows={filteredRows} summary={summary} dateLabel={selectedDate === 'all' ? 'ทั้งหมด' : thaiDateLabel(selectedDate)} organizationLabel={selectedOrg === 'all' ? 'ทุกหน่วยงาน' : selectedOrg}/><a href={sourceUrl} target="_blank" rel="noreferrer">Google Sheet <ExternalLink size={14}/></a></section>
    ...
    {!summary.respondents && !error ? <section className="satv2-empty"><TrendingUp size={38}/><h2>V2 พร้อมรับข้อมูล</h2><p>ตรวจพบ {schema.questionIndexes.length.toLocaleString('th-TH')} ประเด็นประเมินจากหัวคอลัมน์ และ {schema.fieldDefinitions.length.toLocaleString('th-TH')} ฟิลด์ทั้งหมด เมื่อ Google Sheet มีคำตอบ ระบบจะอัปเดตอัตโนมัติประมาณทุก 15 วินาที</p></section> : <>
      ...
      <section className="satv2-insight-grid"><article className="satv2-card"><h2>ประเด็นเด่น</h2><div className="satv2-insight best"><span>คะแนนสูงสุด</span><strong>{summary.best?.label || '—'}</strong><b>{summary.best ? `${summary.best.average.toFixed(2)} / 5` : '—'}</b></div><div className="satv2-insight watch"><span>ควรติดตาม</span><strong>{summary.worst?.label || '—'}</strong><b>{summary.worst ? `${summary.worst.average.toFixed(2)} / 5` : '—'}</b></div></article><article className="satv2-card"><h2>โครงสร้างข้อมูล</h2><dl><div><dt>Sheet Tab</dt><dd>{source.sheetName}</dd></div><div><dt>ฟิลด์ทั้งหมด</dt><dd>{schema.fieldDefinitions.length.toLocaleString('th-TH')} ฟิลด์</dd></div><div><dt>ประเด็นคะแนน</dt><dd>{schema.questionIndexes.length.toLocaleString('th-TH')} ข้อ</dd></div><div><dt>ช่องข้อเสนอแนะ</dt><dd>{schema.commentIndexes.length.toLocaleString('th-TH')} ช่อง</dd></div><div><dt>จำนวนรายการ</dt><dd>{sheetData.rows.length.toLocaleString('th-TH')}</dd></div></dl></article></section>
    </>}

    {!error && <FieldAnalyticsPanel fields={summary.fieldSummaries || []} totalRows={summary.respondents}/>} 
  </div></main>;
}
